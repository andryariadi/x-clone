...
const ImageEditor = ({ onClose, previewURL, settings, setSettings }: ImageEditorProps) => {
      ...
      <div className="rounded-xl p-12 flex flex-col gap-4 py-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <svg width={28} viewBox="0 0 24 24" onClick={onClose} className="cursor-pointer">
              <path fill="#e7e9ea" d="M7.414 13l5.043 5.04-1.414 1.42L3.586 12l7.457-7.46 1.414 1.42L7.414 11H21v2H7.414z" />
            </svg>
            <span className="font-bold text-base">Media Settings</span>
          </div>
          <button className="py-2 px-4 rounded-full bg-white text-black text-sm font-bold" onClick={onClose}>
            Save
          </button>
        </div>
        ...
        <div className="w-[500px] h-[500px] flex items-center">
          {/* Image Preview */}
          <Image src={previewURL} alt="" width={500} height={500} className={`w-full ${settings.type === "original" ? "h-full object-contain" : settings.type === "square" ? "aspect-square object-cover" : "aspect-video object-cover"}`} />
        </div>
